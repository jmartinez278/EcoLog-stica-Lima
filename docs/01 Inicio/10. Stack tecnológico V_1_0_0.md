...
Las alternativas se evaluarán mediante los siguientes criterios:
...
- Leaflet y OpenStreetMap permiten una solución cartográfica con fuerte presencia de tecnologías abiertas.
...
|---|---:|
| MERN | 3.95 / 5 |
| **Python + FastAPI + React + PostgreSQL** | **4.55 / 5** |
| Java + Spring Boot + Angular + PostgreSQL | 4.20 / 5 |
...
El motor de optimización será un componente independiente de la capa HTTP.

Podrá implementar algoritmos y metaheurísticas para resolver variantes de:
...
Este diagrama es preliminar y será refinado en el documento **12. Modelo C4 V_1_0_0.md**.
...
- Motor de optimización desacoplado de la interfaz.
...
La selección de Python + FastAPI no implica automáticamente menor consumo energético que todas las alternativas; su ventaja en este proyecto radica en que permite implementar el motor de optimización con menor complejidad de desarrollo y mantener una arquitectura relativamente liviana para el MVP.
...
El stack seleccionado será utilizado como base para los documentos posteriores de:
